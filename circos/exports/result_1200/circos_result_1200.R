...
species <- "hg38"
gain_file <- "/Users/aminsuliman/Downloads/MPG_database_H2/circos/exports/result_1200/events_gain.tsv"
loss_file <- "/Users/aminsuliman/Downloads/MPG_database_H2/circos/exports/result_1200/events_loss.tsv"
connections_file <- "/Users/aminsuliman/Downloads/MPG_database_H2/circos/exports/result_1200/connections.tsv"
svg_output <- "/Users/aminsuliman/Downloads/MPG_database_H2/circos/output/circos_result_1200.svg"
plot_title <- "CNV and Translocation Circos Plot"
plot_subtitle <- "sample_test_result_id=1200 | genome_build=GRCh38"
...
circos.trackPlotRegion(
  ...
  bg.col = "#e7f0ff",
  panel.fun = function(x, y) {
    circos.lines(CELL_META$xlim, c(0.5, 0.5), col = "#bfd6ff", lwd = 1)
  }
)

if (nrow(connections) > 0) {
  link_width_bp <- 1800000
  link_region <- function(pos) {
    c(max(0, pos - link_width_bp), pos + link_width_bp)
  }
  for (i in seq_len(nrow(connections))) {
    circos.link(
      connections$chr1[i],
      link_region(connections$pos1[i]),
      connections$chr2[i],
      link_region(connections$pos2[i]),
      col = adjustcolor("#71dfc0", alpha.f = 0.88),
      border = adjustcolor("#71dfc0", alpha.f = 0.95),
      h.ratio = 0.65
    )
  }
}
...
legend(
  ...
  fill = c("#004dbb", "#ff9500", adjustcolor("#71dfc0", alpha.f = 0.88)),
  border = NA,
  bty = "n",
  cex = 0.8
)
